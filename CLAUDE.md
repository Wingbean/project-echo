...
ไม่ว่าจะถามด้วยภาษาอะไร ให้ตอบด้วยภาษาไทย ทับศัพท์ภาษาอังกฤษได้ และ ต้องสรุปสิ่งที่นายทำทุกครั้ง
...
There is no SQLite cache, sync job, scheduler, or dashboard of most HosXP data — that machinery was removed once it was clear the old dashboard was unused. The one exception is the name→HN search cache described below. Local state under `instance/` is `barcode_cache.json` (last scanned HN) plus `app.db` — a small SQLite DB that stores the local user-accounts table for the auth system (see Auth model below) and, as of the name-search feature, the `patient_names` cache table too.
...
### `/tcvr` is date-based, not per-HN

`GET /api/tcvr?date=YYYY-MM-DD` (defaults to today) computes Thai CVD Risk % for every patient screened (`opdscreen`) on that one day — ported verbatim from the `labncd` container's `/tcvr` feature (same HosXP host, same formula). It doesn't use `_hn_search`: `app/services/tcvr_service.py:fetch_tcvr_df` runs `sql/tcvr.sql`, then computes the RAMA Thai CVD Risk score in pandas/numpy (blood formula if cholesterol is present, waist/height-ratio formula otherwise) and buckets into Low/Medium/High/VeryHigh/Extreme — constants and formula are intentionally not tunable, since this is a clinical calculation. Gated by plain `@login_required` (no dedicated access flag, unlike `/emr`/`/patient_search`). The `/tcvr` page (`views.py`) auto-fetches today's results on load via `static/js/pages/tcvr.js` (the one page in this app that doesn't wait for a manual search submit) and offers a client-side-only HN filter.

### Name → HN search is the one cached exception

`/api/patient_search` doesn't query HosXP directly — searching `patient.fname`/`lname` live would be a full scan of a ~hundred-thousand-row table on every keystroke-adjacent request. Instead `scripts/sync_patient_names.py` (a standalone script, *not* part of the Flask app process) runs nightly via host crontab (`docker compose exec -T web python scripts/sync_patient_names.py` — must run *inside* the container since `instance/app.db` is root-owned there, a bare host `uv run` can't write to it) and replaces the contents of the local `patient_names` table (`app/models/patient_name.py`, `hn`/`pname`/`fname`/`lname`, same SQLAlchemy `Base` as `User` — lives in `instance/app.db`) with a fresh pull of `sql/patient_search.sql` from HosXP. It deletes-and-reinserts inside one transaction rather than `DROP`/recreate, since gunicorn workers read/write that same SQLite file for auth. The endpoint itself just does a `LIKE`-based OR match against the local cache (`fname` and/or `lname`, both optional, blank field contributes no clause, `LIMIT 100`), gated by the `can_access_name_search` per-user flag (same shape as `can_access_echo`/`can_access_emr`) rather than plain `login_required`, since a name is directly identifying PHI.
...
- **Login flow** (`app/api/routes_auth.py`): `/auth/login` redirects to Google via Authlib, passing `prompt="select_account"` so Google always shows its account chooser instead of silently reusing whichever Google account has an active browser session; `/auth/callback` upserts the `User` row by `google_sub` and sets `session["user_id"]`. On first login, an email-verification link (`itsdangerous`-signed, 24h expiry) is sent via SMTP (`app/utils/email.py`) to `/auth/verify-email/<token>`. Verifying the email is **not** sufficient — an admin must separately set `is_active = True` before the account can use anything.
